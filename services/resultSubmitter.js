...
class ResultSubmitter {
   ...
   */
  async submit(result) {
    this.logger.info(`[ResultSubmitter] Submitting result for task ${result.task_id}`, {
      type: result.type,
    ...
    for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
      try {
        ...
        this.logger.info(`[ResultSubmitter] Task ${result.task_id} submitted successfully`, {
        ...
        this.logger.warn(`[ResultSubmitter] Submission attempt ${attempt}/${this.maxRetries} failed:`, {
  ...
  async _submitWithSignature(result) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.logger.info(`[ResultSubmitter] Submitting to: ${url.href}`);
      ...
      const handleResponse = (res) => {
        ...
        res.on('end', () => {
          ...
          if (res.statusCode >= 300 && res.statusCode < 400) {
            ...
            this.logger.error(`[ResultSubmitter] Unexpected redirect (${res.statusCode}) to: ${redirectUrl}`);
            this.logger.error('[ResultSubmitter] This usually means:');
            this.logger.error('  1. The Laravel endpoint /internal/task-result does not exist');
            this.logger.error('  2. CSRF middleware is interfering (add route to api.php or exclude from CSRF)');
            this.logger.error('  3. Authentication middleware is blocking the request');
            this.logger.error('  4. LARAVEL_INTERNAL_URL in .env is incorrect');
            this.logger.error(`[ResultSubmitter] Current LARAVEL_INTERNAL_URL: ${this.laravelUrl}`);
  ...
  async submitBatch(results) {
    this.logger.info(`[ResultSubmitter] Submitting batch of ${results.length} results`);
