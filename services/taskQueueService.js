...
class TaskQueueService {
  ...
  async enqueueTask(task) {
    try {
      ...
      this.logger.error('[TaskQueueService] Failed to enqueue task:', error);
      throw error;
    }
  }
  ...
  async enqueueBatch(tasks) {
    ...
    try {
      ...
      this.logger.error('[TaskQueueService] Failed to enqueue batch:', error);
      throw error;
    }
  }
  ...
  async getPendingTasks(limit = 10) {
    try {
      ...
      this.logger.error('[TaskQueueService] Failed to get pending tasks:', error);
      return [];
    }
  }
  ...
  async getTask(taskId) {
    try {
      ...
      this.logger.error('[TaskQueueService] Failed to get task:', error);
      return null;
    }
  }
  ...
  async updateTaskStatus(taskId, status, metadata = {}) {
    try {
      ...
      this.logger.error('[TaskQueueService] Failed to update task status:', error);
      throw error;
    }
  }
  ...
  async getStatistics() {
    try {
      ...
      this.logger.error('[TaskQueueService] Failed to get statistics:', error);
  ...
  async cleanupOldTasks(olderThanDays = 7) {
    try {
      ...
      this.logger.error('[TaskQueueService] Failed to cleanup old tasks:', error);
      return 0;
    }
  }
  ...
  async resetStuckTasks(stuckAfterMinutes = 30) {
    try {
      ...
      this.logger.error('[TaskQueueService] Failed to reset stuck tasks:', error);
      return 0;
    }
  }
}
