...
Type definitions are located in the `types/` directory:

- `types/common.ts` - Common runtime types (ApiResponse, Task, Job, ErrorCode)
- `types/errors.ts` - Custom error classes (BrowserError, AppError, ValidationError)
- `types/browser.d.ts` - Browser automation types
...
npm test -- --testPathPattern="browserController"
```
